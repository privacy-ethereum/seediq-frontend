...
import {
  Es256CircuitParams,
  generateES256AgeInputs,
  JwkEcdsaPublicKey,
  PemPublicKey,
} from "./es256";
import { encodeClaims, stringToPaddedBigIntArray } from "./jwt_utils";
...
export function generateAgeJwtCircuitParams(
  params: number[]
): JwtCircuitParams {
  return {
    es256: {
...
export async function generateAgeJwtInputs(
  ...
  let es256Inputs = await generateES256AgeInputs(
    params.es256,
  ...
  for (const match of matches) {
    ...
    assert.ok(index != -1);
    matchSubstring.push(
      stringToPaddedBigIntArray(match, params.maxSubstringLength)
    );
